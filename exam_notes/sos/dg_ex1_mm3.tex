...
\textbf{1. Write the discrete-time version of \eqref{eq:max_eq1}. $\left( \text{Hint: use }\dot{x} = \frac{\Delta
      x_k}{\Delta t}\right)$:}
...
\end{equation}
The constraint  are discretised using the hint given:
\begin{equation}
  \begin{split}
    \dot{x} = \frac{\Delta x_n}{\Delta t} &= - \tanh{(x)} + u \\
     &= -\tanh{(x_n)} + u_k \\
     &\Downarrow \\
...
\textbf{2. Use the discrete maximum principle to write down the state- and costate equations:}
